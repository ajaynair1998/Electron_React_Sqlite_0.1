      ...
      if (this.state.loading === false) {
          ...
          let tempForStorage = this.state.data.map((item) => {
            return (
              <PatientCard
                ...
                patientId={item[0]}
                key={item[0]}
                handleClickViewButton={this.handleClickViewButton}
              />
            );
          });
          console.log(tempForStorage);
          return <div id="chart">{tempForStorage}</div>;
        }
        ...
        // if screen is set to only show todays patients
          ...
          let tempForStorage = this.state.data.map((item) => {
            ...
            if (
              keyword.test(item[1] ? item[1] : "empty") ||
              item[0].toString().match(keyword)
            ) {
              return (
                <PatientCard
                  patientName={item[1]}
        ...
        <div id="controls_and_records">
          <div id="controls">
            ...
            <div id="screen">
              ...
              <button id="today" onClick={this.handleClick}>
                Today's
              </button>
