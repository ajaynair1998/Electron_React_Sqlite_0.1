...
ipcMain.on("give_patient_data", (event, args) => {
  let queryfor_id_name =
    "SELECT Patient_id,Patient_Name,Date from Patient_Data";
...
ipcMain.on("Edit-Profile-View-Page", (event, args) => {
  ...
  keys_in_double_quotes.forEach((item, index) => {
    afterProcessing.push(
      item + "=" + "'" + values_in_double_quotes[index] + "'"
    );
  });

  let queryToSaveEditedView = `UPDATE Patient_Data SET ${afterProcessing.toString()} WHERE Patient_id=${
    args[0]
  }`;
  ...
  database.all(queryToSaveEditedView, (err, rows) => {
    if (err) {
      ...
      // alert in main window
      dialog.showMessageBox({
        message: "Patient Profile Edited Successfully!",
        button: ["Okay"],
        title: "SUCCESS!",
      });
    }
...
ipcMain.on("Delete-Profile", (event, args) => {
  ...
  database.all(queryToDelete, (err, rows) => {
    if (err) {
      ...
      dialog.showMessageBox({
        message: `Profile ${patientName} with id ${patient_id} 
      has been deleted`,
        button: ["Okay"],
        title: "SUCCESS!",
      });
